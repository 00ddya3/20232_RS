...
# author: 임일
# Factorizagion Machines(FM) 구현 (Amazon 데이터)
...
ratings['ProductId'] = ratings['ProductId'].astype("category")
ratings['user_id'] = ratings['UserId'].cat.codes
ratings['item_id'] = ratings['ProductId'].cat.codes
ratings['rating'] = ratings['Score']
ratings = ratings[['user_id', 'item_id', 'Time', 'rating']].astype(int)
ratings['Time'] = ratings['Time'] / 1000000000
...
for i in range(len(x)):
    ...
    x_value.append(1.)
    x_index.append(time_index)                     # Time encoding
    x_value.append(case['Time'])
...
K = 250
fm1 = FM(num_x, K, data, y, alpha=0.0009, beta=0.003, train_ratio=0.75, iterations=900, tolerance=0.0005, l2_reg=True, verbose=True)
result = fm1.test()
