...
# author: 임일
# Keggle의 Amazon 식품 review 데이터

import numpy as np
import pandas as pd
from sklearn.utils import shuffle

ratings = pd.read_csv('C:/RecoSys/Data/reviews2.csv', encoding='latin-1')
ratings = ratings[['UserId', 'ProductId', 'Score']]
...
class NEW_MF():
    ...
    # Test RMSE 계산하는 method t새로 들어온 정보 
    def test_rmse(self):
        error = 0
...
mf = NEW_MF(R_temp, K=220, alpha=0.005, beta=0.075, iterations=1100, tolerance=0.0001, verbose=True)
test_set = mf.set_test(ratings_test)
result = mf.test()
#print(mf.get_one_prediction(1,2),R_temp.loc[1][2])
print(mf.get_one_prediction('A2NO1TXXS9T0EE', 'B000G6RYNE'),R_temp.loc['A2NO1TXXS9T0EE']['B000G6RYNE'])
...
rating_matrix = ratings.pivot(values='rating', index='user_id', columns='item_id')

# 영화 제목 가져오기
i_cols = ['item_id', 'title', 'release date', 'video release date', 'IMDB URL', 
          'unknown', 'Action', 'Adventure', 'Animation', 'Children\'s', 'Comedy', 
          'Crime', 'Documentary', 'Drama', 'Fantasy', 'Film-Noir', 'Horror', 
          'Musical', 'Mystery', 'Romance', 'Sci-Fi', 'Thriller', 'War', 'Western']
items = pd.read_csv('C:/Recosys/Data/u.item', sep='|', names=i_cols, encoding='latin-1')
...
def recommender(user, n_items=10):
    ...
    recommended_items = items.loc[recommendations.index]['title']
